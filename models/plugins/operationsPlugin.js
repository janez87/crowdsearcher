...
module.exports = exports = function( schema ) {
  ...
  schema.methods.addOperations = function( operations, callback ) {
    log.trace( 'Adding operations: %j', operations );
    var _this = this;
    ...
    Operation.create( operations, function( err, ops ) {
      if ( err ) return callback( err );

      log.trace( 'Added operations: %j', ops );

      // Add the operation to the list, unique
      _this.operations.addToSet.apply( _this.operations, ops );
